...
-- 각 부서별(NULL인 경우 부서없음)
-- 사원의 수, 보너스를 받는 사원의 수
-- 총급여, 평균 급여, 최저 급여, 최고 급여 조회
-- 부서별 오름차순 정렬 , 부서없음이 맨위에
SELECT
    NVL(DEPT_CODE, '부서없음') "부서", COUNT(*) "사원의 수", COUNT(BONUS) "보너스 사원 수", TO_CHAR(SUM(SALARY
    ), '999,999,999') "총 급여", TO_CHAR(AVG(SALARY), '999,999,999') "평균 급여", TO_CHAR(MIN(SALARY
    ), '999,999,999') "최저 급여", TO_CHAR(MAX(SALARY), '999,999,999') "최고 급여"
FROM
    EMPLOYEE
GROUP BY
    DEPT_CODE
...
-- 성별 별 (남자/ 여자) 사원의 수 조회
SELECT
    DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남자', 2, '여자') "성별", COUNT(*)
FROM
    EMPLOYEE
GROUP BY
    SUBSTR(EMP_NO, 8, 1);
...
6: ODER BY      컬럼명 | 별칭 | 컬럼순번 [ASC|DESC] [NULLS FIRST|NULLS LAST]

*/    

-- 부서별 평균 급여가 300만원 이상인 직원의 평균 급여를 조회 ( 부서코드, 평균급여 )
SELECT
    NVL(DEPT_CODE, '부서없음') "부서", TO_CHAR(AVG(SALARY), '999,999,999')
FROM
    EMPLOYEE
HAVING
    AVG(SALARY) >= 3000000
...
SELECT
    NVL(JOB_CODE, '부서없음') "직급", TO_CHAR(SUM(SALARY), '999,999,999')
FROM
    EMPLOYEE
HAVING
    SUM(SALARY) >= 10000000
...
/*
    ...
    - ROLLUP(컬럼1, 컬럼2) : 컬럼 1을 가지고 다시 중간 집계를 내는 함수
...
/*
    ...
    UNION(합집합) : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출 (중복 제거)
    UNION ALL(합집합) : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출 (중복 제거 x)
...
-- 1. UNION
-- 부서코드가 D5인 사원 또는 급여가 300만원 초과인 사원들의 사번, 사원명, 부서코드, 급여를 조회
SELECT
    EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM
    EMPLOYEE
