...
/*
 ...
 - 산술 연산을 하게 되면 컬럼명이 지저분해진다. 이때 컬럼명에 별칭을 부여해서 정리
 - 별칭을 부여할 때 띄어쓰기 혹은 특수문자가 별칭에 포함될 경우 반드시 큰따옴표("")로 감싸줘야 한다.
*/
-- EMPLOYEE에 사원명, 연봉, 보너스가 포함된 연봉((급여 + (보너스*급여)) * 12) 조회
SELECT
    EMP_NAME                         "직원명",
    SALARY * 12                      "연봉",
...
/*
LIKE
-비교하고자하는 칼럼값이 내가 제사한 특정 패턴에 만족될 경우 조회
...
-- 이메일 중 _앞글자가 3글자인 사원들의 사번, 이름, 이메일 조회 ex)sim_bs
...
-- 위의 사원들 이외의 사원들 조회 논리 부정연산자: NOT
SELECT
    EMP_ID,
    EMP_NAME,
    EMAIL
FROM
    EMPLOYEE
WHERE
    EMAIL NOT LIKE '___$_%' ESCAPE '$';
    
/*
    IS NULL / IS NOT NULL
    - 컬럼값에 NULL이 있을 경우 NULL값 비교에 사용되는 연산자
    */
 -- EX) 보너스를 받지 않는 사원 (BONUS값이 NULL)의 사번, 이름 급여, 보너스조회
...
-- 4. 급여가 200만원 이상 500만원 이하이고 입사일이 '01/01/01' 이상이고 보너스를 받지 않는 
...
-- 5. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 
-- 사원들의 사번, 사원명, 급여, 보너스포함 연봉 조회 (별칭)
SELECT
    EMP_ID,
    EMP_NAME,
    SALARY,
...
/*
    ORDER BY 절
    - SELECT문 가장 마지막 줄에 작성 뿐만 아니라 실행순서 또한 마지막에 실해
    
    - FROM WHERE SELECT ODERBY  순
    
    [표현법]
    SELECT 컬럼, 컬럼, ...
    FROM 테이블명
    WHERE 조건식
    ODER BY 정렬하고자하는 컬럼값 [ASC|DESC] [NULLS FIRST|NULLS LAST];
    
    - ASC : 오름차순 정렬 (생략시 기본값)
    - DESC : 내림차순 정렬
    
    - NULLS FIRST : 정렬하고자하는 컬럼값에 NULL이 있을 경우 
      해당 데이터를 맨 앞으로 배치 (생략시 DESC일때 기본값)
    - NULLS LAST : 정렬하고자하는 컬럼값에 NULL이 있을 경우
